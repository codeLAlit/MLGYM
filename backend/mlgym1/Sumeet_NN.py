...
def update(mini_batch, eta, lmbda):
    ...
    params.biases = [b * (1 - (lmbda * eta / m)) - (eta / m)
                     * nb for b, nb in zip(params.biases, nabla_b)]
    params.weights = [w * (1 - (lmbda * eta / m)) - (eta / m)
                      * nw for w, nw in zip(params.weights, nabla_w)]
...
def backprop(x, y):
    ...
    # using cross entropy cost, therefore delta is not dependent on
    # sigmoid_prime(z)
    ...
    for l in range(1, len(params.sizes) - 1):
        delta = np.dot(params.weights[-l].transpose(), delta) * \
...
def NN_predict(db, theta, y_val=None):
    ...
    for w in theta:
        ...
        activation = np.concatenate(
            (np.ones((activation.shape[0], 1)), activation), axis=1)
    ...
    if y_val is not None:
        ...
        return correct * 100 / y_val.shape[0]
    else:
